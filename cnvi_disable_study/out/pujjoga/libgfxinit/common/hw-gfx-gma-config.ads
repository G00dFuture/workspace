...
private package HW.GFX.GMA.Config is
   ...
   Gen : constant Generation := Tigerlake;
   ...
   PCH : constant Gen_PCH_Type := Alder_Point;
   ...
   Default_MMIO_Base : constant := 16#af000000#;
   ...
   Gen_Tigerlake     : constant Boolean := Gen = Tigerlake;
   ...
   Tigerlake_On      : constant Boolean := Gen >= Tigerlake;
   ...
   function CPU_Tigerlake return Boolean is
     (Gen_Tigerlake and then CPU = Tigerlake);
   function CPU_Alderlake return Boolean is
   ...
   function Alderlake_On return Boolean is
     (Gen_Tigerlake and then CPU >= Alderlake);
   ...
   Tiger_Point_On    : constant Boolean := Tigerlake_On and then PCH >= Tiger_Point;
   ...
   function Pipe_Enabled_Workaround return Boolean is
     (CPU_Broadwell);
   Has_EDP_Transcoder            : constant Boolean := Haswell_On and not Tigerlake_On;
   ...
   ----------- Transcoder -------
   Need_Early_Transcoder_Setup   : constant Boolean := Tigerlake_On;
   function Need_Pipe_Arb_Slots return Boolean is
     (Alderlake_On);

   ----------- Planes -----------
   Has_Mbus_Dbox_Credits         : constant Boolean := Tigerlake_On;
   Has_Wide_Watermarks           : constant Boolean := Tigerlake_On;
   Has_Plane_Color_Control       : constant Boolean := Tigerlake_On;
   function Has_New_Mbus_Dbox_Credits return Boolean is
     ((Alderlake_On and Is_LP));

   ----------- Pipe -------------
   function Need_Underrun_Rec_Disable return Boolean is
     (Alderlake_On);
   ...
   function Has_New_Type_C_PLL_Enable return Boolean is
     (Alderlake_On);
   function Has_CDClk_PLL_Crawl return Boolean is
     (Alderlake_On);
   ...
   Has_TGL_DDI_Select            : constant Boolean := Tigerlake_On;
   ...
   ----------- GMBUS: -----------
   Ungate_GMBUS_Unit_Level       : constant Boolean := Skylake_On and not Tigerlake_On;
   ...
   function Has_Per_Pipe_SRD return Boolean is
     (Broadwell_On);
   function Has_XELPD_Power_Domains return Boolean is
     (Alderlake_On);
   ...
   ----------- Type-C: ----------
   Has_Type_C_Ports              : constant Boolean := Tigerlake_On;
   function Need_TC_PHY_Ownership return Boolean is
     (Alderlake_On);
   function Need_Loadgen_Select return Boolean is
     (Alderlake_On);
   function Need_DP_Alt_Switch return Boolean is
     (Alderlake_On);
   function Has_TCSS_DDI_Status return Boolean is
     (Alderlake_On);

   ----------- Rawclk -----------
   Need_Rawclk_Numerator         : constant Boolean := Tigerlake_On;

   ----------- Combo Phy --------
   function Has_TGL_Buffer_Translations return Boolean is
     (CPU_Tigerlake);

   -----Early Graphics Init -----
   function Romstage_GFX_Init return Boolean is
     (CPU_Alderlake);
   ...
   function Is_Alder_Lake_N (Device_ID : Word16) return Boolean is
     (Device_Id = 16#46d0# or
      ...
      Device_Id = 16#46d2# or
      Device_Id = 16#46d3# or
      Device_Id = 16#46d4#);
